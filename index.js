...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr)) {
    const result = arr.find((el) => el > num);
    return result;
  } else {
    return undefined;
  }
...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr)) {
    if (arr.findIndex((names) => names === name)) {
      return 1;
    } else {
      return -1;
    }
...
function findLastEvenNumber(arr) {
  ...
  if (Array.isArray(arr)) {
    const result = arr.findLast((paramtr) => {
      paramtr % 2;
    });
    //  else {
    //   return `Парних чисел не знайдено в масиві <<${arr}>>`;
    // }

    return result;
  } else {
    return `Вхідний параметр має бути масивом <<${arr}>>`;
  }
}
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) {
    ...
    return arr.splice(0, startIdx).map((x) => x * 2);
  }
}
...
function sumAndPrint(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let result = arr.reduce((num, num2) => num + num2, value);
    return result;
  }
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const flatMap = arr.flat(1).map((el) => el * el);

    let test = flatMap.filter((value) => value > threshold);
    return test;
  }
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let war1 = arr.slice(n).reduceRight((elem, elem2) => elem + elem2);

    // let war2 = war1.reduceRight((elem, elem2) => elem + elem2);

    // return war2.forEach((elem) => {
    //   console.log(elem);
    // });
...
function expandArrayByN(arr, n) {
  ...
  if (Array.isArray(arr)) {
    // let wars = arr.flatMap((num) => num.repeat(n));
    // return wars;
  } else {
...
function findLongestWord(arr) {
  ...
  //   if (Array.isArray(arr)) {
  //     const longestWord = 0;
  //     arr.reduce((arc) => arc > longestWord, (longestWord = arc));
  //     return longestWord;
  //   } else {
  //     return false`ERROR`;
  //   }
...
function findDuplicateElements(arr) {
  ...
  if (Array.isArray(arr)) {
    // return arr.filter((ar, ar2) => ar.indexOf() === ar2.indexOf());
  } else {
    return [];
  }
}
...
function capitalizeWords(arr) {
  ...
  if (Array.isArray(arr)) {
    let as = arr.map((el) => el.toUpperCase());
    return as;
  } else {
    return [];
  }
...
function calculateTotalPrice(arr) {
  ...
  let price = 0;
  if (Array.isArray(arr)) {
    arr.reduce((name, pri) => price + pri, price);
    return price;
  } else {
    return 0;
  }
